...
export function CategoryExclusionsStep2({ audienceId }: CategoryExclusionsStep2Props) {
    ...
    const allCompleted = channels.length > 0 && channels.every((c) => c.is_completed);
    ...
    const handleSkip = (channelId: string) => {
        ...
        callSkip(
            ...
            () => {
                setChannels((prev) =>
                    prev.map((c) =>
                        c.channel_id === channelId ? { ...c, is_completed: true } : c
                    )
                );
                setSkippingId(null);
            },
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody>
                                            {channels.map((channel) => {
                                                const showActions =
                                                    !channel.is_completed && channel.status === "active";
